...
namespace daxa
{
    ...
    void task_resource_viewer_debug_ui_hook(ImplTaskGraphDebugUi & context, ImplTaskGraph * impl, u32 task_index, daxa::TaskInterface & ti, bool is_pre_task)
    {
        ...
        for (u32 attach_i = 0; attach_i < ti.attachment_infos.size(); ++attach_i)
        {
            ...
            if (task.attachment_access_groups[attach_i].first != &resource->access_timeline[static_cast<u32>(state.timeline_index)])
            {
                continue;
            }
            ...
            if (attachment_info.type == TaskAttachmentType::BUFFER)
            {
                ...
                if (!state.freeze_resource)
                {
                    ...
                    for (auto & child_viewer : state.buffer.child_buffer_inspectors)
                    {
                        ...
                        // I dont care. Its only copied to be displayed later.
                        // Its not important if the data is mixed of two or more frames while reading back.
                        bool recreate_child_clone_buffer = true;
                        if (!child_viewer.second.clone_buffer.is_empty())
                        {
                            ...
                            recreate_child_clone_buffer = clone_buffer_info.size != (child_buffer_info.size);
                        }
                        if (recreate_child_clone_buffer)
                        {
                            if (!child_viewer.second.clone_buffer.is_empty())
                            {
            ...
            if (attachment_info.type == TaskAttachmentType::IMAGE)
            {
...

                {
                    ...
                    clone_image_view_info.slice.base_mip_level = static_cast<u32>(state.image.mip);
                    clone_image_view_info.slice.base_array_layer = static_cast<u32>(state.image.layer);
                ...
                if(work_code.starts_with("["))
                {
                    while(work_code.starts_with("["))
                    {
                        ...
                        for(u32 char_idx = 0; char_idx < array_size_str.size(); ++char_idx)
                        {
                            ...
                            dim_size = dim_size * 10 + static_cast<i32>(array_size_str[char_idx] - '0');
                        }
            ...
            if (begin_return)
            {
                if (context.device.is_id_valid(state.image.clone_image) && state.open_frame_count > READBACK_CIRCULAR_BUFFER_SIZE)
                {
                    ...
                    case ScalarKind::FLOAT:
                        min_value = static_cast<f64>(std::min(
                        ...
                        max_value = static_cast<f64>(std::max(
                    ...
                    if (!state.image.display_image.is_empty())
                    {
                        ...
                        if (ImPlot::BeginPlot("##image", ImVec2(real_width, real_height), ImPlotFlags_NoTitle | ImPlotFlags_NoLegend | ImPlotFlags_NoMouseText))
                        {
                            ...
                            {
                                ImPlot::SetupAxisLimits(ImAxis_X1, static_cast<f64>(context.boardcast_image_viewer_limits_prev_frame->min_x), static_cast<double>(context.boardcast_image_viewer_limits_prev_frame->max_x), ImPlotCond_Always);
                                ImPlot::SetupAxisLimits(ImAxis_Y1, static_cast<f64>(context.boardcast_image_viewer_limits_prev_frame->min_y), static_cast<double>(context.boardcast_image_viewer_limits_prev_frame->max_y), ImPlotCond_Always);
                            }
        ...
        else if (resource.kind == TaskResourceKind::BUFFER)
        {
            ...
            if (context.device.is_id_valid(state.buffer.clone_buffer) && state.open_frame_count > READBACK_CIRCULAR_BUFFER_SIZE)
            {
                ...
                auto compile_and_cache = [&]() -> CompileBufferStructFormatResult
                {
                    ...
                    if (result.success)
                    {
                        ...
                        if(context.buffer_layout_cache_folder.has_value())
                        {
                            ...
                            if (outfile.is_open())
                            {
                                outfile.write(state.buffer.format_c_struct_code.data(), static_cast<std::streamsize>(state.buffer.format_c_struct_code.find_first_of('\0') + 1));
                                outfile.close();
                            }
                        }
                    }
                ...
                auto field_element_size_align = [&](TgDebugStructFieldDefinition const & field_def) -> std::pair<u64, u64>
                {
                    ...
                    TgDebugTypeDefinition const & field_type = field_def.type_index < 0 ? TG_DEBUG_PRIMITIVE_DEFINITIONS[static_cast<u32>(abs(field_def.type_index) - 1)] : state.buffer.tg_debug_struct_definitions.at(static_cast<u32>(field_def.type_index));
                ...
                auto calc_memory_size_align = [&](i32 type_index, auto calc_memory_size_align_ref)
                {
                    ...
                    TgDebugTypeDefinition & type = state.buffer.tg_debug_struct_definitions.at(static_cast<u32>(type_index));
                    ...
                    for (u32 field_i = 0; field_i < type.fields.size(); ++field_i)
                    {
                        ...
                        if (field_def.pointer_depth > 0)
                        {
                            field_size = static_cast<u64>(field_def.array_size) * sizeof(void*);
                            field_alignment = alignof(void*);
                        }
                        else
                        {
                            TgDebugTypeDefinition const & field_type = field_def.type_index < 0 ? TG_DEBUG_PRIMITIVE_DEFINITIONS[static_cast<u32>(abs(field_def.type_index) - 1)] : state.buffer.tg_debug_struct_definitions.at(static_cast<u32>(field_def.type_index));
                            field_size = static_cast<u64>(field_def.array_size) * field_type.size;
                ...
                if (state.buffer.tg_debug_struct_definitions.empty())
                {
                    ...
                    calc_memory_size_align(static_cast<i32>(state.buffer.tg_debug_struct_definitions.size() - 1), calc_memory_size_align);
                }
                ...
                {
                    TgDebugTypeDefinition const & type_def = field_def.type_index < 0 ? TG_DEBUG_PRIMITIVE_DEFINITIONS[static_cast<u32>(abs(field_def.type_index)-1)] : type_defs[static_cast<u32>(field_def.type_index)];
                    ...
                    if(ImGui::BeginTable("leaf", 4, primitive_flags))
                    {
                        ...
                        if (is_pointer)
                        {
                            std::string text = std::format("{}{}", type_def.name, std::string(static_cast<u32>(field_def.pointer_depth), '*'));
                ...
                auto draw_type = [&](TgDebugTypeDefinition const& type_def, std::string const & in_parent_field_name, u32 array_index, u64 memory_offset, auto & draw_type_ref, std::byte* readback_ptr, u64 max_memory_offset) -> void
                {
                    ...
                    ImGui::PushID(static_cast<int>(array_index));
                    ...
                    {
                        for(u32 field_i = 0; field_i < type_def.fields.size(); ++field_i)
                        {
                            ImGui::Indent(STRUCT_NEST_INDENTATION);
                            ImGui::PushID(static_cast<i32>(field_i));
                            ...
                            TgDebugTypeDefinition const & field_type_def = is_primitive_type ? TG_DEBUG_PRIMITIVE_DEFINITIONS[static_cast<u32>(abs(field_def.type_index) - 1)] : type_defs[static_cast<u32>(field_def.type_index)];
                            ...
                            if (should_draw_field)
                            {
                                ...
                                while (clipper.Step())
                                {
                                    ...
                                    {
                                        u64 array_element_memory_offset = memory_offset + field_def.in_struct_memory_offset + field_element_size * static_cast<u64>(array_i);
                                        if (is_primitive_type || field_def.pointer_depth > 0)
                                        {
                                            draw_leaf(field_def, static_cast<u32>(array_i), array_element_memory_offset, readback_ptr, max_memory_offset);
                                        }
                                        else
                                        {
                                            draw_type_ref(field_type_def, field_def.name, is_array ? static_cast<u32>(array_i) : ~0u, array_element_memory_offset, draw_type_ref, readback_ptr, max_memory_offset);
                                        }
                                    }
                                }
                                if (is_array)
